...
const Cart = ({ setShowCart }: PropsType) => {
    return import { RxCross1 } from "react-icons/rx";
    import CartProduct from "./CartProduct"
    import { useAppSelector } from "@/redux/hooks"; 
} */
}
...
const Cart = ({ setShowCart }: any) => {
  ...
  return (
    <div
      className="bg-[#0000007d] w-full min-h-screen fixed left-0 top-0 z-20 
  overflow-y-scroll"
    >
      <div className="max-w-[400px] w-full min-h-full bg-white absolute right-0 top-0 p-6">
        ...
        <div className="mt-6 space-y-2">
          {products?.map((item: any) => (
            <CartProduct key={item.id} id={item.id} img={item.img} tittle={item.tittle} price={item.price} quantity={item.quantity} />
          ))}
        </div>
        <div className="flex justify-between items-start font-medium text-xl py-4">
          <p>Total:</p>
          <p>${getTotal()}.00</p>
        </div>
